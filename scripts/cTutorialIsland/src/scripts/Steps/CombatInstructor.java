...
public class CombatInstructor implements Task {
    ...
    RSTile combatInstructorTile =  new RSTile(3106, 9505, 0);
    NPCStep combatStep = new NPCStep(3307, new RSTile(3106, 9505, 0));

    private void talkToInstructor(){
        if (combatInstructorTile.distanceTo(Player.getPosition()) < 4){
            if(NpcChat.talkToNPC(3307)){
                NPCInteraction.waitForConversationWindow();
                NPCInteraction.handleConversation();
            }
        }
        else {
            PathingUtil.walkToTile(combatInstructorTile);
        }
    }

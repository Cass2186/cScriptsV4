...
import static scripts.Tasks.KourendFavour.ArceuusLibrary.Constants.Rooms.*;
        ...
        for (Room nextRoom : route) {
            ...
            while (attempts <= 1) {
                ...
                if (currentRoom.getRoomLinkTypeToRoom(nextRoom) == RoomLinkType.WALK) {
                    if (PathingUtil.walkToTile(nextRoom.getArea().getRandomTile())) {
                        Timer.waitCondition(() -> isInRoom(nextRoom), 6000, 9000);
                    } else

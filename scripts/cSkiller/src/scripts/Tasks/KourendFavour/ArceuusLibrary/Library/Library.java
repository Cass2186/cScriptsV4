package scripts.Tasks.KourendFavour.ArceuusLibrary.Library;


import org.tribot.api2007.types.RSTile;
import scripts.Tasks.KourendFavour.ArceuusLibrary.Statistics;
import scripts.Tasks.KourendFavour.ArceuusLibrary.Walking.Room;
...
import static scripts.Tasks.KourendFavour.ArceuusLibrary.Library.Book.*;

/**
 * Credit here goes to Runelite - this is adapated from their plugin
 * Credit to JustJ
 */
public class Library {

  //  private static final JustLogger LOGGER = new JustLogger(Library.class);

    private final HashMap<RSTile, Bookcase> byPoint = new HashMap<>();
    private final HashMap<Integer, ArrayList<Bookcase>> byLevel = new HashMap<>();
    private final HashMap<Room, ArrayList<Bookcase>> byArea = new HashMap<>();
    ...
    private synchronized void reset() {
       ...
       // LOGGER.info("Library is now reset");
    }

    public synchronized void mark(RSTile loc, Book book) {
        Bookcase bookcase = byPoint.get(loc);
        if (bookcase == null) {
            //LOGGER.debug(String.format("Requested non-existent bookcase at %s", loc));
            return;
        }
        ...
        if (bookcase.isBookSet()) {
            ...
            //    LOGGER.debug(String.format("We expected %s but found %s", bookcase.getBook(), book));
                reset();
            }
            ...
            if (book == null && !bookcase.getPossibleBooks().isEmpty() && bookcase.getPossibleBooks().stream().noneMatch(Book::isDarkManuscript)) {
              //  LOGGER.debug("We found nothing");
                reset();
            }

            if (book != null && !bookcase.getPossibleBooks().contains(book)) {
              //  LOGGER.debug(String.format("We found a new book at %s", loc));
                reset();
            }
        }
        ...
        //LOGGER.info(String.format("Setting bookcase %s to %s", bookcase.getIndex(), book));
        for (; ; ) {
                    ...
                    if (i % step == 0) {
                        ...
                        if (iBookcase.isBookSet() && seqI < sequence.size()) {
                            ...
                            if (!((isSeqManuscript && iBookcase.getBook() == null) || (iBookcase.getBook() == seqBook))) {
                               // LOGGER.debug(String.format("Bailing @ i=%s ai=%s %s; %s != %s", i, ai, iBookcase.getIndex(),
                        ...
                        if (iBookcase.isBookSet() && iBookcase.getBook() != null && iBookcase.getIndex().size() == 1) {
                           // LOGGER.debug(String.format("Bailing @ i=%s ai=%s %s; %s is set", i, ai, iBookcase.getIndex(),
                           //         iBookcase.getBook()));
                            found = 0;
                            break;
                        }
           ...
           // LOGGER.info(String.format("Number of each sequence is now %s", Arrays.toString(certainty)));
